  ...
  async connect(
    ...
    if (this._client)
      if (connected) {
        ...
        const subscriberOptStreamConfig: StreamConfig = {
          ...defaultStreamConfiguration,
          name: 'SUBSCRIBER-OPT',
          subjects: ['subscriber:opt'],
        };

        const notificationStatusStreamConfig: StreamConfig = {
          ...defaultStreamConfiguration,
          name: "NOTIFICATION_STATUS",
          subjects: ["notificationStatus:created"],
        };

        const customerResponseStreamConfig: StreamConfig = {
          ...defaultStreamConfiguration,
          name: "CUSTOMER_RESPONSE",
          subjects: ["customerResponse:created"],
        };


        const replyStreamConfig: StreamConfig = {
          ...defaultStreamConfiguration,
          name: 'REPLY',
          subjects: ['reply:created'],
        };
        ...
        if (typeof jsm == "object" && jsm.hasOwnProperty("streams")) {
          await jsm.streams
            .add(notificationStatusStreamConfig)
            .then(() =>
              console.log(
                "notificationStatusStreamConfig has been configured ✅"
              )
            )
            .catch((e: any) =>
              console.log("notificationStatusStreamConfig", e)
            );

          await jsm.streams
            .add(customerResponseStreamConfig)
            .then(() =>
              console.log("customerResponseStreamConfig has been configured ✅")
            )
            .catch((e: any) =>
              console.log("customerResponseStreamConfig", e)
            );

          await jsm.streams
            .add(subscriberOptStreamConfig)
            .then(() =>
              console.log(
                "subscriberOptStreamConfig has been configured ✅",
              ),
            )
            .catch((e: any) => console.log("replyStreamConfig", e));

          await jsm.streams
            .add(replyStreamConfig)
            .then(() =>
              console.log("replyStreamConfig has been configured ✅"),
            )
            .catch((e: any) => console.log("replyStreamConfig", e));
        }
