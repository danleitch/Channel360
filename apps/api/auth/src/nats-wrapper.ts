	...
	async connect(
		...
		if (this._client)
			if (connected) {
				...
				const userStreamConfig: StreamConfig = {
					...defaultStreamConfiguration,
					name: 'USER',
					subjects: ['user:created', 'user:updated'],
				}

				const adminStreamConfig: StreamConfig = {
					...defaultStreamConfiguration,
					name: 'ADMIN',
					subjects: ['admin:created'],
				}

				if (typeof jsm == 'object' && jsm.hasOwnProperty('streams')) {
					await jsm.streams.add(userStreamConfig).catch((e) => console.log(e))
					await jsm.streams.add(adminStreamConfig).catch((e) => console.log(e))
				}
