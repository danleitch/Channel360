...
export const ExportLogsForm: FC = () => {
  ...
  const onSubmit = handleSubmit(async (data) => {
    try {
      ...
      await loggingService.exportLogs(orgId, startDate, endDate).then((res) => {
        // This will download the the exported file
        handleExport(res.data, 'logging');
      });
      reset();
      setOpen(false);
    } catch (error) {
  ...
  return (
    <>
      <Popup title="Export Logs" header="Choose a date range." open={open} setOpen={setOpen}>
