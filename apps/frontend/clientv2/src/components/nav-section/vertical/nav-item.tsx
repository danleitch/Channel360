...
// Note: Custom function for navigation
const useActiveLink = (path: string) => {
  const currentPath = usePathname();

  const isActive = currentPath.startsWith(`${path}/`) || currentPath === path;
...
const NavItem = forwardRef<HTMLDivElement, NavItemProps>(
  (
    ...
    // Note: isActive is custom logic that might break the MUI templates intended use cases
    const isActive = useActiveLink(path);
    const subItem = depth !== 1;

    const renderContent = (
      <StyledNavItem
        data-cy={`nav-item-${title.toLowerCase()}-button`}
        ref={ref}
        disableGutters
        open={open}
        depth={depth}
